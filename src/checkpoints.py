...
def save_checkpoint_best(model, logging_path, best_epoch, end_training=False):
    """
    save the best training checkpoint acoording the validation's loss
    """
    model_name = 'model.pth'
    if not(end_training):       # at the end of one epoch
        torch.save(model.state_dict(), os.path.join(logging_path, model_name))
        
    else:                       # at the end of the training
        old_name = os.path.join(logging_path, model_name)
